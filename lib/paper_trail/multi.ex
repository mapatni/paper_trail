defmodule PaperTrail.Multi do
  ...
  defp make_version_struct(%{event: "insert"}, model, options) do
    ...
    skip_attrs = module_or_changeset_struct(model)
    ...
    %Version{
      ...
      item_changes: Map.drop(serialize(model), skip_attrs),
      originator_id:
  ...
  defp make_version_struct(%{event: "update"}, changeset, options) do
    originator = PaperTrail.RepoClient.originator()
    originator_ref = options[originator[:name]] || options[:originator]
    skip_attrs = module_or_changeset_struct(changeset)
                 |> SkippedAttributesDetector.call()
  ...
  defp make_version_struct(%{event: "delete"}, model_or_changeset, options) do
    originator = PaperTrail.RepoClient.originator()
    originator_ref = options[originator[:name]] || options[:originator]
    skip_attrs = module_or_changeset_struct(model_or_changeset)
                 |> SkippedAttributesDetector.call()
    ...
    %Version{
      ...
      item_changes: Map.drop(serialize(model_or_changeset), skip_attrs),
      originator_id:
        case originator_ref do
          nil -> nil
          _ -> originator_ref |> Map.get(:id)
  ...
  defp module_or_changeset_struct(%Ecto.Changeset{data: data}) do
    data.__struct__
  end
  defp module_or_changeset_struct(model) do
    model.__struct__
  end
